...
const std::regex REGEXP_FLOAT("^[+-]?([0-9]+([.][0-9]*)?|[.][0-9])$");
const std::regex REGEXP_INT("[+-]?[0-9]+");
const std::regex REGEXP_BOOL("(1|0)");
...
struct SKU{
    ...
    float SKU_promotion_price = 0;
    float SKU_stock; // Остаток на складе
    int SKU_type = 0; // 1 - весовой; 0 - штучный
};
...
string bool_check(string num) {
    if (regex_match(num, REGEXP_BOOL)) {
        return num;
    } else {
...
string float_check(string num){
    if(regex_match(num, REGEXP_FLOAT)) {
        return num;
    } else {
...
int main(){
    ...
    ******************************/
    ...
    cout << "Сейчас в базе " << stock.size() << " товара.\n\nПерейти к покупкам или добавить новый товар? \n1 - да, 0 - нет: ";
    cin >> free_string;
    if(stoi(bool_check(free_string))) {
        while (stoi(S_flag)) {
            S_flag = true;

            cout << "Введите название товара: ";
            cin >> stock_struct.SKU_name;

            cout << "Введите цену на товар: ";
            cin >> free_string;
            stock_struct.SKU_price = stof(float_check(free_string));

            cout << "Это весовой товар? 1 - да, 0 - нет: ";
            cin >> free_string;
            if (stoi(bool_check(free_string))) {
                stock_struct.SKU_type = 1;
            }

            cout << "Это акционный товар? 1 - да, 0 - нет: ";
            cin >> free_string;
            if (stoi(bool_check(free_string))) {
                cout << "Цена по акции: ";
                cin >> free_string;
                stock_struct.SKU_promotion_price = stof(float_check(free_string));
            }

            cout << "Сколько товара добавить на склад? ";
            cin >> free_string;
            stock_struct.SKU_stock = stof(float_check(free_string));
        ...
        cout << "\x1B[2J\x1B[H"; // Очистка консоли(видимость очистки) Ps просто листает вниз и устанавливает курсор в левый верхний угол
        print_struct(stock);
    }
